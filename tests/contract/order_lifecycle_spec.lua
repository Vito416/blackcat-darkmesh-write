...
local function assert_ok(resp)
  assert(resp and resp.status == "OK", ("expected OK, got %s"):format(resp and resp.status or "nil"))
end

local function assert_err(resp, code)
  assert(resp and resp.status == "ERROR", ("expected ERROR, got %s"):format(resp and resp.status or "nil"))
  if code then
...
local function run_lifecycle()
  reset()
  local create = run("CreateOrder", { cartId = "cart-1", customerId = "cust-1", siteId = "site-1", currency = "USD", orderId = "order-1" })
  assert_ok(create)
  ...
  local confirm = run("UpsertOrderStatus", { orderId = orderId, status = "confirmed", expectedVersion = 1 })
  assert_ok(confirm)
  ...
  local fulfilled = run("UpsertOrderStatus", { orderId = orderId, status = "fulfilled", expectedVersion = 3 })
  assert_ok(fulfilled)
  assert(state.orders[orderId].status == "fulfilled")
  assert(state.orders[orderId].version == 4)

  local returned = run("UpsertOrderStatus", { orderId = orderId, status = "returned", expectedVersion = 4 })
  assert_ok(returned)
  assert(state.orders[orderId].status == "returned")
  assert(state.orders[orderId].version == 5)

  local refunded = run("UpsertOrderStatus", { orderId = orderId, status = "refunded", expectedVersion = 5 })
  assert_ok(refunded)
  assert(state.orders[orderId].status == "refunded")
  assert(state.orders[orderId].version == 6)

  local conflict = run("UpsertOrderStatus", { orderId = orderId, status = "refunded", expectedVersion = 1 })
