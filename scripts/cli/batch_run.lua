...
local function run_fixture(path)
  ...
  if cmd.commands then
    ...
    if expected_str then
      expected_all = cjson.decode(expected_str)
    end
  end
  ...
  if expected_str or expected_all then
    ...
    if not same(results, expected) then
...
for file in lfs.dir(fixtures_dir) do
  if file:match "%.json$" and not file:match "%.expected%.json$" then
    ...
    if body then
      ...
      if ok and decoded and decoded._env then
        env_over = decoded._env
      end
    end
    local env_prefix = ""
    ...
    local cmd = string.format(
      "%sLUA_PATH=%q LUA_CPATH=%q lua5.4 %s %s",
      env_prefix,
