...
local function stable_encode(val)
  ...
  if t == "string" then return cjson.encode(val) end
  if t == "table" then
    if is_array(val) then
      local parts = {}
      for i = 1, #val do parts[#parts+1] = stable_encode(val[i]) end
...
local function legacy_payload(ev)
  return (ev.siteId or "") .. "|" .. (ev.pageId or ev.orderId or "") .. "|" .. (ev.versionId or ev.amount or "")
end
...
assert(not ok_full_wrong and why_full_wrong == "hmac_mismatch", "legacy hmac must fail in full mode")
