...
local cart = write.route({ Action='CartAddItem', ['Request-Id']='eo-1', ['Actor-Role']='admin', nonce='ne1', ts=os.time(), payload={ cartId='cart_eo', siteId='s1', currency='USD', sku='sku-eo', qty=1, price=1000 } })
assert(ok(cart), 'cart add failed')
local order = write.route({ Action='CreateOrder', ['Request-Id']='eo-2', ['Actor-Role']='admin', nonce='ne2', ts=os.time(), payload={ orderId='ord_eo', cartId='cart_eo', siteId='s1', currency='USD' } })
assert(ok(order), 'order failed')
-- publish page version
local pub = write.route({ Action='PublishPageVersion', ['Request-Id']='eo-3', ['Actor-Role']='admin', nonce='ne3', ts=os.time(), payload={ siteId='s1', pageId='home', versionId='v1', manifestTx='tx-eo' } })
